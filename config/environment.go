...
type Environment struct {
	Server struct {
		...
		TLSKey      string `env:"OMEGA_TLS_KEY,required"`
		TLSCert     string `env:"OMEGA_TLS_CERT,required"`
		TimeZone    string `env:"OMEGA_TIME_ZONE,required"`
	}
	...
	Database struct {
		DSN  string `env:"OMEGA_DATABASE_URL,required"`
